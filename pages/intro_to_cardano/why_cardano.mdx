...
There methods by which blockchains come to agreement about the correct state of the blockchain are known as consensus mechanisms. Cardano operates using a proof-of-stake consensus mechanism called Ouroboros. This is a good choice for energy efficiency because Ouroboros uses a tiny fraction of the energy of Bitcoin's proof-of-work consensus while still enjoying the same security guarantees. 
...
### Superior Staking
Cardano's Proof-of-Stake design is one of the best in crypto. The concept of staking is that you point, or delegate, your ADA toward a block producing stake pool that you trust. The value of stake delegated to a given stake pool is how the Ouroboros mechanism determines which pool should add the next block to the blockchain. In return for this service the pool, and the users that delegate to that pool, receive an ADA reward. Staking provides security to the Cardano system because it makes it extremely expensive for dishonest nodes attack the Cardano system because they have to overcome the high value of ADA staked to honest pools.

Users benefit from staking ADA because they receive rewards every 5 days. Staking is nearly risk-free: Staked ADA never leaves your wallet and can be spent freely anytime. [Over 60% of ADA currently in circulation is staked](https://lookerstudio.google.com/u/0/reporting/3136c55b-635e-4f46-8e4b-b8ab54f2d460/page/k5r9B?s=huXqeQmi9qA). This broad participation contributes to the healthy security and decentralization of the network. Furthermore, the ongoing staking rewards make ADA an attractive long-term asset to hold because users receive passive income.   
...
Cardano has a committed and highly active community. Some of the main hubs of activity are [X](), [Twitter](), [Youtube](). There are also active [forums and discord channels]() to engage with the latest happenings in the ecosystem. 
...
#### Scalable 
Scalability refers to the speed and volume of transactions that is possible on the network. The Cardano network has been designed from the start with scalability in mind. 

Currently the Cardano network can handle many transactions while preserving decentralization. While this is sufficient for now, there are some initiatives happening that will further upgrade Cardano's ability to handle more transactions. These include [Hydra](https://hydra.family/head-protocol/), [input endorsers](https://cexplorer.io/article/understanding-input-endorsers), network parameter increases, side chains, and layer 2s. Multiple strategies are being implemented so Cardano is ready for further adoption. [See details about some of them here](https://cardano-scaling.org/).
